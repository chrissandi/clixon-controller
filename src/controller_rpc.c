                      ...
                      void         *regarg)
{
    ...
    while ((cv = cvec_each(devvec, cv)) != NULL){
        ...
        case DT_ACTIONS:
            ...
            if (clixon_xml2cbuf1(cb, xroot1, 0, 0, NULL, -1, 0, 0, WITHDEFAULTS_REPORT_ALL) < 0)
                goto done;
            break;
        case DT_SYNCED:
        case DT_TRANSIENT:
            ...
            if (clixon_xml2cbuf1(cb, xroot, 0, 0, NULL, -1, 0, 0, WITHDEFAULTS_EXPLICIT) < 0)
                goto done;
            if (xroot)
                xroot = NULL;
            break;
...
/*! Collect result from a device-rpc and remove data when collected
 *
 * Also honor NACM read access, that is remove data that violates NACM read access
 * @param[in]  h       Clixon handle
 * @param[in]  xe      Request: <rpc><xn></rpc>
 * @param[out] cbret   Return xml tree, eg <rpc-reply>..., <rpc-error..
 ...
 */
static int
rpc_device_rpc_result(clixon_handle h,
                      cxobj        *xe,
                      cbuf         *cbret,
                      void         *arg,
                      void         *regarg)
{
    ...
    if (controller_transaction_result_get(h, ct, cbret) < 0)
        goto done;
 ok:
    retval = 0;
 done:
...
controller_rpc_init(clixon_handle h)
{
    ...
    if (rpc_callback_register(h, rpc_transactions_actions_done,
        ...
        goto done;
    if (rpc_callback_register(h, rpc_device_rpc_result,
                              NULL,
                              CONTROLLER_NAMESPACE,
                              "device-rpc-result"
                              ) < 0)
